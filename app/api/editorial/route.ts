import { addArticleComment, applyWorkflowAction, getAdminArticle, getEditorialReport, listAdminUsers, listArticleComments, listArticleRevisions, listWorkflowEvents, restoreArticleRevision } from "../../../db";
import { canAccessArticle, canEditArticle } from "../../../db/editorial-permissions.mjs";
...
export async function GET(request: Request) {
  ...
  const article = getAdminArticle(articleId); if (!article) return Response.json({ error: "Haber bulunamadı." }, { status: 404 });
  if (!canAccessArticle(auth.user!.role, auth.user!.id, article)) return Response.json({ error: "Yalnızca size atanmış haberleri görüntüleyebilirsiniz." }, { status: 403 });
...
export async function POST(request: Request) {
  ...
  if (payload.type === "comment") { const auth = authorizeAdmin(request, ["admin", "publisher", "editor", "reporter"]); if (auth.response) return auth.response; const article = getAdminArticle(Number(payload.articleId)); if (!article || !canAccessArticle(auth.user!.role, auth.user!.id, article)) return Response.json({ error: "Bu haber için işlem yetkiniz bulunmuyor." }, { status: 403 }); try { const id = addArticleComment(Number(payload.articleId), String(payload.note ?? ""), auth.user!); return Response.json({ ok: true, id }, { status: 201 }); } catch { return Response.json({ error: "Yorum en az iki karakter olmalı." }, { status: 400 }); } }
  if (payload.type === "restore") { const auth = authorizeAdmin(request, ["admin", "publisher", "editor"]); if (auth.response) return auth.response; const article = getAdminArticle(Number(payload.articleId)); if (!article || !canEditArticle(auth.user!.role, auth.user!.id, article)) return Response.json({ error: "Bu haberin sürümünü geri yükleme yetkiniz bulunmuyor." }, { status: 403 }); try { return Response.json({ ok: true, article: restoreArticleRevision(Number(payload.articleId), Number(payload.revisionId), auth.user!) }); } catch { return Response.json({ error: "Revizyon geri yüklenemedi." }, { status: 409 }); } }
  if (payload.type === "workflow" && payload.action && payload.action in actionRoles) { const auth = authorizeAdmin(request, actionRoles[payload.action]); if (auth.response) return auth.response; const article = getAdminArticle(Number(payload.articleId)); if (!article || !canAccessArticle(auth.user!.role, auth.user!.id, article)) return Response.json({ error: "Bu haber için işlem yetkiniz bulunmuyor." }, { status: 403 }); const requiredNote = ["request_changes", "reject", "withdraw", "correction"].includes(payload.action); if (requiredNote && String(payload.note ?? "").trim().length < 4) return Response.json({ error: "Bu işlem için açıklama zorunludur." }, { status: 400 }); try { return Response.json({ ok: true, article: applyWorkflowAction(Number(payload.articleId), payload.action, String(payload.note ?? ""), auth.user!, payload.assigneeId) }); } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "İşlem tamamlanamadı." }, { status: 400 }); } }
