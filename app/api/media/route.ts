...
export async function GET() { return Response.json({ media: listMediaAssets(), stats: { ...getMediaStats(), quotaBytes: quotaBytes() } }); }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const media = saveMediaAsset({ ...stored, originalName: file.name.slice(0, 180), altText: String(form.get("altText") ?? "").trim().slice(0, 240), credit: String(form.get("credit") ?? "").trim().slice(0, 160) });
