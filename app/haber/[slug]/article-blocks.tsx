...
export function ArticleVideoPlayer({ src, title = "Haber videosu", preload = "metadata" }: { src: string; title?: string; preload?: "none" | "metadata" }) {
...
export function ArticleBlocks({ blocks, excludeVideoUrl = "", lazyImages = false }: { blocks: ContentBlock[]; excludeVideoUrl?: string; lazyImages?: boolean }) {
  return <>{blocks.map((block) => {
    ...
    if (block.type === "image" && safeMedia(block.content)) return <figure className="article-inline-media" key={block.id}><img src={block.content} alt={block.caption || "Haber görseli"} loading={lazyImages ? "lazy" : undefined} />{block.caption && <figcaption>{block.caption}</figcaption>}</figure>;
