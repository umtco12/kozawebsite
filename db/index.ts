...
export type HeadlinePosition = "left-top" | "left-bottom" | "right-top" | "right-bottom";
export type HomepagePlacement = "latest" | "slider" | "side" | "below";
...
export type ArticleRecord = { id: number; slug: string; title: string; spot: string; body: string; bodyHtml: string; blocks: ContentBlock[]; category: string; status: ArticleStatus; workflowState: WorkflowState; assignedTo: number | null; editVersion: number; correctionNote: string; withdrawnAt: number | null; heroImage: string; imageAlt: string; videoUrl: string; author: string; sourceName: string; sourceUrl: string; seoTitle: string; seoDescription: string; isBreaking: number; isFeatured: number; isHomepageGallery: number; homepageOrder: number; homepagePlacement: HomepagePlacement; headlinePosition: HeadlinePosition; publishedAt: number | null; scheduledAt: number | null; agencySourceId: number | null; agencyExternalId: string; agencyCredit: string; agencyReceivedAt: number | null; agencyDisclaimer: string; agencyEditorialLock: number; agencyUpdatePending: number; createdBy: string; updatedBy: string; createdAt: number; updatedAt: number };
export type PhotoGalleryRecord = ArticleRecord & { galleryImages: { src: string; caption: string }[] };
export type ArticleInput = Omit<ArticleRecord, "id" | "createdAt" | "updatedAt" | "publishedAt" | "scheduledAt" | "homepagePlacement" | "headlinePosition" | "createdBy" | "updatedBy"> & { id?: number; publishedAt?: number | string | null; scheduledAt?: number | string | null; homepagePlacement?: HomepagePlacement; headlinePosition?: HeadlinePosition };
...
export type HomepageLayoutValue = { slider: number[]; side: number[]; below: number[]; latest?: number[] };

export class HomepageLayoutConflictError extends Error {
  constructor() { super("Ana sayfa düzeni başka bir yönetici tarafından değiştirildi."); this.name = "HomepageLayoutConflictError"; }
}
...
function mapHeadlinePosition(value: unknown): HeadlinePosition { return ["left-top", "left-bottom", "right-top", "right-bottom"].includes(String(value)) ? value as HeadlinePosition : "left-bottom"; }
function mapHomepagePlacement(value: unknown, legacyFeatured: unknown = 0): HomepagePlacement { return ["latest", "slider", "side", "below"].includes(String(value)) ? value as HomepagePlacement : Number(legacyFeatured) ? "slider" : "latest"; }
function mapArticle(row: Record<string, unknown>): ArticleRecord { const body = String(row.body); return { id: Number(row.id), slug: String(row.slug), title: String(row.title), spot: String(row.spot), body, bodyHtml: String(row.body_html || ""), blocks: parseBlocks(row.content_blocks, body), category: String(row.category), status: row.status as ArticleStatus, workflowState: (row.workflow_state || (row.status === "published" ? "published" : "reporter_draft")) as WorkflowState, assignedTo: row.assigned_to == null ? null : Number(row.assigned_to), editVersion: Number(row.edit_version || 1), correctionNote: String(row.correction_note || ""), withdrawnAt: row.withdrawn_at == null ? null : Number(row.withdrawn_at), heroImage: String(row.hero_image), imageAlt: String(row.image_alt), videoUrl: String(row.video_url), author: String(row.author), sourceName: String(row.source_name), sourceUrl: String(row.source_url), seoTitle: String(row.seo_title), seoDescription: String(row.seo_description), isBreaking: Number(row.is_breaking), isFeatured: Number(row.is_featured), isHomepageGallery: Number(row.is_homepage_gallery || 0), homepageOrder: Number(row.homepage_order ?? 100), homepagePlacement: mapHomepagePlacement(row.homepage_placement, row.is_featured), headlinePosition: mapHeadlinePosition(row.headline_position), publishedAt: row.published_at == null ? null : Number(row.published_at), scheduledAt: row.scheduled_at == null ? null : Number(row.scheduled_at), agencySourceId: row.agency_source_id == null ? null : Number(row.agency_source_id), agencyExternalId: String(row.agency_external_id || ""), agencyCredit: String(row.agency_credit || ""), agencyReceivedAt: row.agency_received_at == null ? null : Number(row.agency_received_at), agencyDisclaimer: String(row.agency_disclaimer || ""), agencyEditorialLock: Number(row.agency_editorial_lock || 0), agencyUpdatePending: Number(row.agency_update_pending || 0), createdBy: String(row.created_by || ""), updatedBy: String(row.updated_by || ""), createdAt: Number(row.created_at), updatedAt: Number(row.updated_at) }; }
...
function ensureColumn(db: InstanceType<typeof Database>, table: string, name: string, definition: string) { const columns = db.prepare(`PRAGMA table_info(${table})`).all() as { name: string }[]; if (columns.some((column) => column.name === name)) return false; db.exec(`ALTER TABLE ${table} ADD COLUMN ${name} ${definition}`); return true; }

/* Haberi ilk ekleyen ve son güncelleyen kişinin adı. Kolonlar ilk kez eklendiğinde arşivdeki
   haberler için sürüm geçmişinden bir kez türetilir; haberin içeriğine dokunulmaz. */
...
function ensureSchema(db: InstanceType<typeof Database>) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS articles (
      ...
      is_breaking INTEGER NOT NULL DEFAULT 0 CHECK (is_breaking IN (0,1)), is_featured INTEGER NOT NULL DEFAULT 0 CHECK (is_featured IN (0,1)), is_homepage_gallery INTEGER NOT NULL DEFAULT 0 CHECK (is_homepage_gallery IN (0,1)), homepage_order INTEGER NOT NULL DEFAULT 100, homepage_placement TEXT NOT NULL DEFAULT 'latest' CHECK (homepage_placement IN ('latest','slider','side','below')), headline_position TEXT NOT NULL DEFAULT 'left-bottom' CHECK (headline_position IN ('left-top','left-bottom','right-top','right-bottom')), published_at INTEGER, scheduled_at INTEGER, content_blocks TEXT NOT NULL DEFAULT '[]', workflow_state TEXT NOT NULL DEFAULT 'reporter_draft', assigned_to INTEGER, edit_version INTEGER NOT NULL DEFAULT 1, correction_note TEXT NOT NULL DEFAULT '', withdrawn_at INTEGER, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL
    );
  ...
  /* Zengin metin gövdesi. Mevcut haberlerde boş kalır; boşken yayında eski blok düzeni gösterilir. */
  ensureColumn(db, "articles", "body_html", "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, "articles", "created_by", "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, "articles", "updated_by", "TEXT NOT NULL DEFAULT ''");
  if (!db.prepare("SELECT 1 FROM content_items WHERE key='_article_actors_v2'").get()) {
    db.transaction(() => {
      backfillArticleActors(db);
      db.prepare("INSERT INTO content_items(key,value,updated_at) VALUES('_article_actors_v2','true',?)").run(Date.now());
    })();
  }
  ...
  ensureColumn(db, "articles", "homepage_placement", "TEXT NOT NULL DEFAULT 'latest'");
  ensureHomepageLatestSchema(db);
  ensureColumn(db, "articles", "headline_position", "TEXT NOT NULL DEFAULT 'left-bottom'");
  ensureColumn(db, "articles", "is_homepage_gallery", "INTEGER NOT NULL DEFAULT 0 CHECK (is_homepage_gallery IN (0,1))");
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_articles_homepage_gallery ON articles(is_homepage_gallery,status,published_at DESC) WHERE is_homepage_gallery=1");
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_articles_homepage_placement ON articles(homepage_placement,status,homepage_order,published_at DESC)");
  ...
  if (articleCount === 0 && demoEnabled) {
    const now = Date.now();
    const insert = db.prepare(`INSERT INTO articles (slug,title,spot,body,category,status,hero_image,image_alt,video_url,author,source_name,source_url,seo_title,seo_description,is_breaking,is_featured,is_homepage_gallery,homepage_placement,published_at,scheduled_at,created_at,updated_at) VALUES (@slug,@title,@spot,@body,@category,@status,@heroImage,@imageAlt,'',@author,@sourceName,@sourceUrl,@title,@spot,@isBreaking,@isFeatured,@isHomepageGallery,@homepagePlacement,@publishedAt,NULL,@createdAt,@createdAt)`);
    db.transaction(() => { for (const article of seedArticles) insert.run({ ...article, isHomepageGallery: "isHomepageGallery" in article && article.isHomepageGallery ? 1 : 0, homepagePlacement: article.isFeatured ? "slider" : "latest", publishedAt: article.status === "published" ? now - article.publishedOffsetMinutes * 60_000 : null, createdAt: now - Math.max(article.publishedOffsetMinutes, 1) * 60_000 }); })();
  }

  /* Yeni ana sayfa konumları ilk kez açıldığında var olan manşet seçimleri korunur.
     En güncel normal haber manşet yanına, bir sonraki haber manşet altına alınır;
     bu yalnız bir kez çalışır ve sonradan yapılan editör tercihlerini ezmez. */
  const homepageLayoutSeedKey = "_homepage_placements_v1";
  if (!db.prepare("SELECT 1 FROM site_settings WHERE key=?").get(homepageLayoutSeedKey)) {
    const now = Date.now();
    db.transaction(() => {
      db.prepare("UPDATE articles SET homepage_placement='slider' WHERE is_featured=1").run();
      const candidates = db.prepare("SELECT id FROM articles WHERE status='published' AND homepage_placement='latest' ORDER BY published_at DESC LIMIT 2").all() as { id: number }[];
  ...
  /* Önceki panel sürümünde Manşet seçilen haberlere varsayılan sıra 100 verildiği için
     ilk beş slayda giremiyorlardı. Mevcut kayıtlar bir kez, en son seçilen en önde
     olacak biçimde gerçek manşet sırasına taşınır. */
  repairUnrankedSliderArticles(db);
  normalizeHomepagePlacementLimits(db);
...
export function getDb() { if (!database) { const path = databasePath(); mkdirSync(dirname(path), { recursive: true }); database = new Database(path); database.function("koza_search_text", { deterministic: true }, archiveSearchText); database.pragma("journal_mode = WAL"); database.pragma("foreign_keys = ON"); database.pragma("busy_timeout = 8000"); ensureSchema(database); } return database; }
...
type ArticleArchiveOptions = { status?: ArticleStatus; category?: string; assignedTo?: number; search?: string; limit?: number; offset?: number };
export function listArticles(options: ArticleArchiveOptions = {}) {
  const { sql, params } = articleArchiveFilter(options);
  const limit = Number.isFinite(options.limit) ? Math.min(Math.max(Math.floor(options.limit!), 1), 100) : 50;
  const offset = Number.isFinite(options.offset) ? Math.max(0, Math.floor(options.offset!)) : 0;
  return (getDb().prepare(`SELECT a.*,EXISTS(SELECT 1 FROM agency_items ai WHERE ai.article_id=a.id AND ai.status='pending_update') AS agency_update_pending FROM articles a ${sql} ORDER BY COALESCE(a.published_at,a.scheduled_at,a.updated_at) DESC,a.id DESC LIMIT ? OFFSET ?`).all(...params, limit, offset) as Record<string, unknown>[]).map(mapArticle);
}
export function countArchiveArticles(options: ArticleArchiveOptions = {}) {
  const { sql, params } = articleArchiveFilter(options);
  return (getDb().prepare(`SELECT count(*) AS total FROM articles a ${sql}`).get(...params) as { total: number }).total;
}
...
function publishDueArticles() {
  ...
  const db = getDb();
  db.transaction(() => {
    const due = db.prepare("SELECT id,homepage_placement AS placement FROM articles WHERE status='scheduled' AND scheduled_at IS NOT NULL AND scheduled_at<=? ORDER BY scheduled_at,id").all(now) as { id: number; placement: HomepagePlacement }[];
    const publish = db.prepare("UPDATE articles SET status='published',workflow_state='published',published_at=COALESCE(published_at,scheduled_at,?),edit_version=edit_version+1,updated_at=? WHERE id=?");
    for (const article of due) {
      publish.run(now, now, article.id);
      if (article.placement === "latest") recordHomepageLatestEntries(db, [article.id], now);
      else promoteHomepageArticle(db, article.id, article.placement);
...
export function listHomepageArticles(placement: HomepagePlacement, limit = 10) {
  ...
  const order = placement === "latest" ? homepageLatestOrderSql : "homepage_order ASC,published_at DESC,id DESC";
  return (getDb().prepare(`SELECT * FROM articles WHERE status='published' AND homepage_placement=? ORDER BY ${order} LIMIT ?`).all(placement, safeLimit) as Record<string, unknown>[]).map(mapArticle);
}

function homepageLayoutRevision(db: InstanceType<typeof Database>) {
  const rows = db.prepare(`SELECT id,homepage_placement AS placement,homepage_order AS position,updated_at AS updatedAt
    FROM articles WHERE status='published' AND homepage_placement IN ('slider','side','below')
    ORDER BY homepage_placement,homepage_order,id`).all();
...
export function getHomepageLayout() {
  publishDueArticles();
  const db = getDb();
  const selected = (db.prepare(`SELECT * FROM articles WHERE status='published' AND homepage_placement IN ('slider','side','below')
    ORDER BY CASE homepage_placement WHEN 'slider' THEN 1 WHEN 'side' THEN 2 ELSE 3 END,homepage_order,published_at DESC`).all() as Record<string, unknown>[]).map(mapArticle);
  const latest = (db.prepare(`SELECT * FROM articles WHERE status='published' AND homepage_placement='latest' ORDER BY ${homepageLatestOrderSql} LIMIT 60`).all() as Record<string, unknown>[]).map(mapArticle);
  return {
    revision: homepageLayoutRevision(db),
    layout: {
      slider: selected.filter((article) => article.homepagePlacement === "slider"),
      side: selected.filter((article) => article.homepagePlacement === "side"),
      below: selected.filter((article) => article.homepagePlacement === "below"),
      latest,
    },
  };
}

export function saveHomepageLayout(layout: HomepageLayoutValue, expectedRevision: string, actor: AdminUser) {
  const db = getDb();
  const now = Date.now();
  const selectedIds = [...layout.slider, ...layout.side, ...layout.below];
  const transaction = db.transaction(() => {
    const currentRevision = homepageLayoutRevision(db);
    if (!expectedRevision || currentRevision !== expectedRevision) throw new HomepageLayoutConflictError();

    if (selectedIds.length) {
      const placeholders = selectedIds.map(() => "?").join(",");
      const published = Number((db.prepare(`SELECT COUNT(*) AS count FROM articles WHERE status='published' AND id IN (${placeholders})`).get(...selectedIds) as { count: number }).count);
      if (published !== selectedIds.length) throw new Error("HOMEPAGE_ARTICLE_NOT_FOUND");
    }

    const before = db.prepare("SELECT id,homepage_placement AS placement,homepage_order AS position FROM articles WHERE status='published' AND homepage_placement IN ('slider','side','below') ORDER BY homepage_placement,homepage_order,id").all();
    const beforeIds = new Set((before as { id: number }[]).map((article) => Number(article.id)));
    const removedIds = [...beforeIds].filter((id) => !selectedIds.includes(id));
    const previewOrder = (layout.latest ?? []).filter((id) => removedIds.includes(id));
    const arrivalOrder = [...previewOrder, ...removedIds.filter((id) => !previewOrder.includes(id))];
    moveHomepageArticlesToLatest(db, arrivalOrder, now, actor.fullName);
    const place = db.prepare("UPDATE articles SET homepage_placement=?,homepage_order=?,is_featured=?,edit_version=edit_version+1,updated_at=? WHERE id=? AND status='published'");
    for (const placement of ["slider", "side", "below"] as const) {
      layout[placement].forEach((id, index) => place.run(placement, index + 1, placement === "slider" ? 1 : 0, now, id));
    }
    const normalized = normalizeHomepagePlacementLimits(db, now);
    const movedToLatest = arrivalOrder;
    for (const id of normalized.movedToLatest) if (!movedToLatest.includes(id)) movedToLatest.push(id);
    db.prepare("INSERT INTO audit_logs (entity_type,entity_id,action,actor,detail,created_at) VALUES ('homepage_layout',0,'reorder',?,?,?)")
      .run(actor.fullName, JSON.stringify({ before, after: layout, movedToLatest }), now);
    return movedToLatest;
  });

  const movedToLatest = transaction();
  return { ...getHomepageLayout(), movedToLatest };
}
...
export function saveArticle(input: ArticleInput, actor: AdminUser | string = "Yayın Yönetmeni") {
  ...
  const workflowState: WorkflowState = input.status === "published" ? "published" : input.status === "scheduled" ? "approved" : input.status === "review" && ["reporter_draft", "changes_requested", "rejected"].includes(currentWorkflowState) ? "editor_review" : currentWorkflowState;
...
function publicationChecklist(article: ArticleRecord) { const missing: string[] = []; if (article.title.trim().length < 12) missing.push("başlık"); if (article.spot.trim().length < 24) missing.push("spot"); if (article.body.trim().length < 1) missing.push("haber metni"); if (!article.heroImage.trim()) missing.push("kapak görseli"); if (!article.imageAlt.trim()) missing.push("görsel alt metni"); if (missing.length) throw new Error(`YAYIN_KONTROL_LISTESI: ${missing.join(", ")}`); }
...
export function applyWorkflowAction(articleId: number, action: string, note: string, actor: AdminUser, assigneeId?: number | null) {
  ...
  return db.transaction(() => {
    const now = Date.now(); const row = db.prepare("SELECT * FROM articles WHERE id=?").get(articleId) as Record<string, unknown> | undefined; if (!row) throw new Error("ARTICLE_NOT_FOUND"); const article = mapArticle(row);
    ...
    if (action === "assign") { db.prepare("UPDATE articles SET assigned_to=?,updated_by=?,updated_at=? WHERE id=?").run(assigneeId ?? null, actor.fullName, now, articleId); }
    else if (action === "correction") { if (article.workflowState !== "published") throw new Error("INVALID_TRANSITION"); db.prepare("UPDATE articles SET correction_note=?,updated_by=?,edit_version=edit_version+1,updated_at=? WHERE id=?").run(note.trim().slice(0, 1000), actor.fullName, now, articleId); db.prepare("INSERT INTO workflow_events (article_id,action,from_state,to_state,actor_id,actor_name,note,created_at) VALUES (?,?,?,?,?,?,?,?)").run(articleId, action, article.workflowState, article.workflowState, actor.id, actor.fullName, note.trim().slice(0, 1000), now); }
    else { const next = transitions[action]; if (!next) throw new Error("INVALID_ACTION"); const allowedFrom: Record<string, WorkflowState[]> = { submit_review: ["reporter_draft", "changes_requested", "rejected"], request_changes: ["editor_review"], approve: ["editor_review"], reject: ["editor_review", "approved"], publish: ["approved"], withdraw: ["published"], reopen: ["rejected", "withdrawn"] }; if (!allowedFrom[action]?.includes(article.workflowState)) throw new Error("INVALID_TRANSITION"); if (["approve", "publish"].includes(action)) publicationChecklist(article); const status: ArticleStatus = next === "published" ? "published" : next === "editor_review" ? "review" : "draft"; if (next === "published" && article.homepagePlacement !== "latest") promoteHomepageArticle(db, articleId, article.homepagePlacement); db.prepare("UPDATE articles SET workflow_state=?,status=?,correction_note=CASE WHEN ?='withdrawn' THEN ? ELSE correction_note END,withdrawn_at=CASE WHEN ?='withdrawn' THEN ? ELSE NULL END,published_at=CASE WHEN ?='published' THEN COALESCE(published_at,?) ELSE published_at END,updated_by=?,edit_version=edit_version+1,updated_at=? WHERE id=?").run(next, status, next, note.trim(), next, now, next, now, actor.fullName, now, articleId); if (next === "published" && article.homepagePlacement === "latest") recordHomepageLatestEntries(db, [articleId], now); normalizeHomepagePlacementLimits(db, now); db.prepare("INSERT INTO workflow_events (article_id,action,from_state,to_state,actor_id,actor_name,note,created_at) VALUES (?,?,?,?,?,?,?,?)").run(articleId, action, article.workflowState, next, actor.id, actor.fullName, note.trim().slice(0, 1000), now); }
...
export function upsertAgencyArticle(source: NewsSource, input: AgencyArticleInput) {
  ...
  return db.transaction(() => {
    ...
    if (id) {
      ...
      db.prepare(`UPDATE articles SET slug=@slug,title=@title,spot=@spot,body=@body,body_html=@bodyHtml,content_blocks=@contentBlocks,updated_by=@actorName,category=@category,status=@status,workflow_state=@workflowState,hero_image=@heroImage,image_alt=@imageAlt,video_url=@videoUrl,author=@author,source_name=@sourceName,source_url=@sourceUrl,seo_title=@title,seo_description=@spot,published_at=COALESCE(published_at,@publishedAt),agency_credit=@agencyCredit,agency_received_at=@now,edit_version=edit_version+1,updated_at=@now WHERE id=@id`).run({ ...values, id });
    } else id = Number(db.prepare(`INSERT INTO articles (slug,title,spot,body,body_html,content_blocks,created_by,updated_by,category,status,workflow_state,hero_image,image_alt,video_url,author,source_name,source_url,seo_title,seo_description,is_breaking,is_featured,homepage_order,published_at,scheduled_at,agency_source_id,agency_external_id,agency_credit,agency_received_at,agency_editorial_lock,created_at,updated_at) VALUES (@slug,@title,@spot,@body,@bodyHtml,@contentBlocks,@actorName,@actorName,@category,@status,@workflowState,@heroImage,@imageAlt,@videoUrl,@author,@sourceName,@sourceUrl,@title,@spot,0,0,500,@publishedAt,NULL,${source.id},@externalId,@agencyCredit,@now,${source.publishMode === "review" ? 1 : 0},@now,@now)`).run(values).lastInsertRowid);
...
export function applyLegacyHomepageOrder(urls: string[]) {
  return syncLegacyHomepagePlacements(getDb(), urls);
}
...
export function listHomepagePhotoGalleries(limit = 8): PhotoGalleryRecord[] {
  publishDueArticles();
  const rows = getDb().prepare("SELECT * FROM articles WHERE status='published' AND is_homepage_gallery=1 ORDER BY published_at DESC,id DESC").all() as Record<string, unknown>[];
  return selectHomepagePhotoGalleries(rows.map(mapArticle), limit) as PhotoGalleryRecord[];
}
