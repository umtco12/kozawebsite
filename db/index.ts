...
import { createPostgresDatabase } from "./postgres-adapter.mjs";
...
export function getDb() {
  if (!database) {
    if (process.env.KOZA_DATABASE_URL) {
      database = createPostgresDatabase(process.env.KOZA_DATABASE_URL) as unknown as InstanceType<typeof Database>;
    } else {
      const path = databasePath();
      mkdirSync(dirname(path), { recursive: true });
      const sqlite = new Database(path);
      sqlite.function("koza_search_text", { deterministic: true }, archiveSearchText);
