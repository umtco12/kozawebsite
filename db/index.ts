...
export function saveArticle(input: ArticleInput, actor: AdminUser | string = "Yayın Yönetmeni") {
  ...
  const existingWorkflow = input.id ? db.prepare("SELECT workflow_state AS workflowState FROM articles WHERE id=?").get(input.id) as { workflowState: WorkflowState } | undefined : undefined;
