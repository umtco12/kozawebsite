...
export type HomepageLayoutValue = { slider: number[]; side: number[]; below: number[] };
...
function publishDueArticles() { const now = Date.now(); const db = getDb(); const result = db.prepare("UPDATE articles SET status='published',workflow_state='published',published_at=COALESCE(published_at,scheduled_at,?),edit_version=edit_version+1,updated_at=? WHERE status='scheduled' AND scheduled_at IS NOT NULL AND scheduled_at<=?").run(now, now, now); if (result.changes) normalizeHomepagePlacementLimits(db, now); }
...
export function getHomepageLayout() {
  ...
  const latest = (db.prepare("SELECT * FROM articles WHERE status='published' AND homepage_placement='latest' ORDER BY published_at DESC,id DESC LIMIT 60").all() as Record<string, unknown>[]).map(mapArticle);
  return {
...
export function saveHomepageLayout(layout: HomepageLayoutValue, expectedRevision: string, actor: AdminUser) {
  ...
  const transaction = db.transaction(() => {
    ...
    db.prepare("UPDATE articles SET homepage_placement='latest',homepage_order=100,is_featured=0,updated_at=? WHERE status='published' AND homepage_placement IN ('slider','side','below')").run(now);
    const place = db.prepare("UPDATE articles SET homepage_placement=?,homepage_order=?,is_featured=?,updated_at=? WHERE id=? AND status='published'");
    ...
    const movedToLatest = [...beforeIds].filter((id) => !selectedIds.includes(id));
...
export function applyLegacyHomepageOrder(urls: string[]) {
  ...
  let matched = 0;
  db.transaction(() => {
    db.prepare("UPDATE articles SET is_featured=0,is_breaking=0,homepage_order=500,homepage_placement='latest' WHERE source_name='kozatv.com.tr'").run();
    ...
    ordered.forEach((url, index) => {
      matched += update.run(index < 5 ? 1 : 0, index === 0 ? 1 : 0, index + 1, index < 5 ? "slider" : "latest", Date.now(), url).changes;
    });
  })();
  return matched;
}
