...
import { inlineHtmlToPlainText, normalizeArticleHtml, normalizeInlineHtml } from "./rich-text.mjs";
...
function ensureSchema(db: InstanceType<typeof Database>) {
  ...
  /* Zengin başlık ve spot. Boşken yayında düz metin gösterilir; arşivdeki haberler etkilenmez. */
  ensureColumn(db, "articles", "title_html", "TEXT NOT NULL DEFAULT ''");
  ensureColumn(db, "articles", "spot_html", "TEXT NOT NULL DEFAULT ''");
...
export function saveArticle(rawInput: ArticleInput, actor: AdminUser | string = "Yayın Yönetmeni") {
  /* Zengin başlık ve spot kaydedildiğinde düz metin karşılıkları onlardan üretilir;
     böylece adres, sekme başlığı, paylaşım kartı ve RSS her zaman tutarlı olur. */
  const titleHtml = normalizeInlineHtml(rawInput.titleHtml);
  const spotHtml = normalizeInlineHtml(rawInput.spotHtml);
  const input: ArticleInput = { ...rawInput, titleHtml, spotHtml,
    title: titleHtml ? inlineHtmlToPlainText(titleHtml) : rawInput.title,
    spot: spotHtml ? inlineHtmlToPlainText(spotHtml) : rawInput.spot };
...
export function applyWorkflowAction(articleId: number, action: string, note: string, actor: AdminUser, assigneeId?: number | null) {
  ...
  return db.transaction(() => {
    ...
    else { const next = transitions[action]; if (!next) throw new Error("INVALID_ACTION"); const allowedFrom: Record<string, WorkflowState[]> = { submit_review: ["reporter_draft", "changes_requested", "rejected"], request_changes: ["editor_review"], approve: ["editor_review"], reject: ["editor_review", "approved"], publish: ["approved"], withdraw: ["published"], reopen: ["rejected", "withdrawn"] }; if (!allowedFrom[action]?.includes(article.workflowState)) throw new Error("INVALID_TRANSITION"); if (["approve", "publish"].includes(action)) publicationChecklist(article); const status: ArticleStatus = next === "published" ? "published" : next === "editor_review" ? "review" : "draft"; if (next === "published" && article.homepagePlacement !== "latest") promoteHomepageArticle(db, articleId, article.homepagePlacement); db.prepare("UPDATE articles SET workflow_state=?,status=?,correction_note=CASE WHEN ?='withdrawn' THEN ? ELSE correction_note END,withdrawn_at=CASE WHEN ?='withdrawn' THEN ? ELSE NULL END,published_at=CASE WHEN ?='published' THEN COALESCE(published_at,?) ELSE published_at END,edit_version=edit_version+1,updated_at=? WHERE id=?").run(next, status, next, note.trim(), next, now, next, now, now, articleId); if (next === "published" && article.homepagePlacement === "latest") recordHomepageLatestEntries(db, [articleId], now); normalizeHomepagePlacementLimits(db, now); db.prepare("INSERT INTO workflow_events (article_id,action,from_state,to_state,actor_id,actor_name,note,created_at) VALUES (?,?,?,?,?,?,?,?)").run(articleId, action, article.workflowState, next, actor.id, actor.fullName, note.trim().slice(0, 1000), now); }
...
export function upsertAgencyArticle(source: NewsSource, input: AgencyArticleInput) {
  ...
  return db.transaction(() => {
    ...
    if (id) {
      ...
      db.prepare(`UPDATE articles SET slug=@slug,title=@title,title_html=@titleHtml,spot=@spot,spot_html=@spotHtml,body=@body,body_html=@bodyHtml,content_blocks=@contentBlocks,category=@category,status=@status,workflow_state=@workflowState,hero_image=@heroImage,image_alt=@imageAlt,video_url=@videoUrl,author=@author,source_name=@sourceName,source_url=@sourceUrl,seo_title=@title,seo_description=@spot,published_at=COALESCE(published_at,@publishedAt),agency_credit=@agencyCredit,agency_received_at=@now,edit_version=edit_version+1,updated_at=@now WHERE id=@id`).run({ ...values, id });
    } else id = Number(db.prepare(`INSERT INTO articles (slug,title,title_html,spot,spot_html,body,body_html,content_blocks,category,status,workflow_state,hero_image,image_alt,video_url,author,source_name,source_url,seo_title,seo_description,is_breaking,is_featured,homepage_order,published_at,scheduled_at,agency_source_id,agency_external_id,agency_credit,agency_received_at,agency_editorial_lock,created_at,updated_at) VALUES (@slug,@title,@titleHtml,@spot,@spotHtml,@body,@bodyHtml,@contentBlocks,@category,@status,@workflowState,@heroImage,@imageAlt,@videoUrl,@author,@sourceName,@sourceUrl,@title,@spot,0,0,500,@publishedAt,NULL,${source.id},@externalId,@agencyCredit,@now,${source.publishMode === "review" ? 1 : 0},@now,@now)`).run(values).lastInsertRowid);
