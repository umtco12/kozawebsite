...
export function upsertAgencyArticle(source: NewsSource, input: AgencyArticleInput) {
  ...
  return db.transaction(() => {
    ...
    if (id) {
      ...
      db.prepare(`UPDATE articles SET slug=@slug,title=@title,spot=@spot,body=@body,body_html=@bodyHtml,content_blocks=@contentBlocks,category=@category,status=@status,workflow_state=@workflowState,hero_image=@heroImage,image_alt=@imageAlt,video_url=@videoUrl,author=@author,source_name=@sourceName,source_url=@sourceUrl,seo_title=@title,seo_description=@spot,published_at=COALESCE(published_at,@publishedAt),agency_credit=@agencyCredit,agency_received_at=@now,edit_version=edit_version+1,updated_at=@now WHERE id=@id`).run({ ...values, id });
    } else id = Number(db.prepare(`INSERT INTO articles (slug,title,spot,body,body_html,content_blocks,category,status,workflow_state,hero_image,image_alt,video_url,author,source_name,source_url,seo_title,seo_description,is_breaking,is_featured,homepage_order,published_at,scheduled_at,agency_source_id,agency_external_id,agency_credit,agency_received_at,agency_editorial_lock,created_at,updated_at) VALUES (@slug,@title,@spot,@body,@bodyHtml,@contentBlocks,@category,@status,@workflowState,@heroImage,@imageAlt,@videoUrl,@author,@sourceName,@sourceUrl,@title,@spot,0,0,500,@publishedAt,NULL,${source.id},@externalId,@agencyCredit,@now,${source.publishMode === "review" ? 1 : 0},@now,@now)`).run(values).lastInsertRowid);
