...
const mustChangePassword = argument("force-change", "1") !== "0";
...
if (process.env.KOZA_DATABASE_URL) {
  const client = new pg.Client({ connectionString: process.env.KOZA_DATABASE_URL, application_name: "kozatv-create-admin" });
  ...
  try {
    await client.query(
      `INSERT INTO admin_users (email,full_name,password_hash,role,active,must_change_password,failed_attempts,locked_until,last_login_at,created_at,updated_at)
       VALUES ($1,$2,$3,$4,1,$5,0,NULL,NULL,$6,$6)
       ON CONFLICT(email) DO UPDATE SET full_name=excluded.full_name,password_hash=excluded.password_hash,role=excluded.role,active=1,must_change_password=excluded.must_change_password,failed_attempts=0,locked_until=NULL,updated_at=excluded.updated_at`,
      [email, fullName, passwordHash, role, mustChangePassword ? 1 : 0, now],
    );
  } finally {
    await client.end();
  }
  ...
  db.prepare("INSERT INTO admin_users (email,full_name,password_hash,role,active,must_change_password,failed_attempts,locked_until,last_login_at,created_at,updated_at) VALUES (?,?,?,?,1,?,0,NULL,NULL,?,?) ON CONFLICT(email) DO UPDATE SET full_name=excluded.full_name,password_hash=excluded.password_hash,role=excluded.role,active=1,must_change_password=excluded.must_change_password,failed_attempts=0,locked_until=NULL,updated_at=excluded.updated_at").run(email, fullName, passwordHash, role, mustChangePassword ? 1 : 0, now, now);
  db.close();
}
